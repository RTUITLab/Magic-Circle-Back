// Code generated by entc, DO NOT EDIT.

package ent

import (
	"context"
	"fmt"

	"entgo.io/ent/dialect/sql"
	"entgo.io/ent/dialect/sql/sqlgraph"
	"entgo.io/ent/schema/field"
	"github.com/0B1t322/Magic-Circle/ent/admin"
	"github.com/0B1t322/Magic-Circle/ent/direction"
	"github.com/0B1t322/Magic-Circle/ent/institute"
	"github.com/0B1t322/Magic-Circle/ent/predicate"
)

// InstituteUpdate is the builder for updating Institute entities.
type InstituteUpdate struct {
	config
	hooks    []Hook
	mutation *InstituteMutation
}

// Where appends a list predicates to the InstituteUpdate builder.
func (iu *InstituteUpdate) Where(ps ...predicate.Institute) *InstituteUpdate {
	iu.mutation.Where(ps...)
	return iu
}

// SetName sets the "name" field.
func (iu *InstituteUpdate) SetName(s string) *InstituteUpdate {
	iu.mutation.SetName(s)
	return iu
}

// AddDirectionIDs adds the "Directions" edge to the Direction entity by IDs.
func (iu *InstituteUpdate) AddDirectionIDs(ids ...int) *InstituteUpdate {
	iu.mutation.AddDirectionIDs(ids...)
	return iu
}

// AddDirections adds the "Directions" edges to the Direction entity.
func (iu *InstituteUpdate) AddDirections(d ...*Direction) *InstituteUpdate {
	ids := make([]int, len(d))
	for i := range d {
		ids[i] = d[i].ID
	}
	return iu.AddDirectionIDs(ids...)
}

// AddAdminIDs adds the "Admins" edge to the Admin entity by IDs.
func (iu *InstituteUpdate) AddAdminIDs(ids ...int) *InstituteUpdate {
	iu.mutation.AddAdminIDs(ids...)
	return iu
}

// AddAdmins adds the "Admins" edges to the Admin entity.
func (iu *InstituteUpdate) AddAdmins(a ...*Admin) *InstituteUpdate {
	ids := make([]int, len(a))
	for i := range a {
		ids[i] = a[i].ID
	}
	return iu.AddAdminIDs(ids...)
}

// Mutation returns the InstituteMutation object of the builder.
func (iu *InstituteUpdate) Mutation() *InstituteMutation {
	return iu.mutation
}

// ClearDirections clears all "Directions" edges to the Direction entity.
func (iu *InstituteUpdate) ClearDirections() *InstituteUpdate {
	iu.mutation.ClearDirections()
	return iu
}

// RemoveDirectionIDs removes the "Directions" edge to Direction entities by IDs.
func (iu *InstituteUpdate) RemoveDirectionIDs(ids ...int) *InstituteUpdate {
	iu.mutation.RemoveDirectionIDs(ids...)
	return iu
}

// RemoveDirections removes "Directions" edges to Direction entities.
func (iu *InstituteUpdate) RemoveDirections(d ...*Direction) *InstituteUpdate {
	ids := make([]int, len(d))
	for i := range d {
		ids[i] = d[i].ID
	}
	return iu.RemoveDirectionIDs(ids...)
}

// ClearAdmins clears all "Admins" edges to the Admin entity.
func (iu *InstituteUpdate) ClearAdmins() *InstituteUpdate {
	iu.mutation.ClearAdmins()
	return iu
}

// RemoveAdminIDs removes the "Admins" edge to Admin entities by IDs.
func (iu *InstituteUpdate) RemoveAdminIDs(ids ...int) *InstituteUpdate {
	iu.mutation.RemoveAdminIDs(ids...)
	return iu
}

// RemoveAdmins removes "Admins" edges to Admin entities.
func (iu *InstituteUpdate) RemoveAdmins(a ...*Admin) *InstituteUpdate {
	ids := make([]int, len(a))
	for i := range a {
		ids[i] = a[i].ID
	}
	return iu.RemoveAdminIDs(ids...)
}

// Save executes the query and returns the number of nodes affected by the update operation.
func (iu *InstituteUpdate) Save(ctx context.Context) (int, error) {
	var (
		err      error
		affected int
	)
	if len(iu.hooks) == 0 {
		affected, err = iu.sqlSave(ctx)
	} else {
		var mut Mutator = MutateFunc(func(ctx context.Context, m Mutation) (Value, error) {
			mutation, ok := m.(*InstituteMutation)
			if !ok {
				return nil, fmt.Errorf("unexpected mutation type %T", m)
			}
			iu.mutation = mutation
			affected, err = iu.sqlSave(ctx)
			mutation.done = true
			return affected, err
		})
		for i := len(iu.hooks) - 1; i >= 0; i-- {
			if iu.hooks[i] == nil {
				return 0, fmt.Errorf("ent: uninitialized hook (forgotten import ent/runtime?)")
			}
			mut = iu.hooks[i](mut)
		}
		if _, err := mut.Mutate(ctx, iu.mutation); err != nil {
			return 0, err
		}
	}
	return affected, err
}

// SaveX is like Save, but panics if an error occurs.
func (iu *InstituteUpdate) SaveX(ctx context.Context) int {
	affected, err := iu.Save(ctx)
	if err != nil {
		panic(err)
	}
	return affected
}

// Exec executes the query.
func (iu *InstituteUpdate) Exec(ctx context.Context) error {
	_, err := iu.Save(ctx)
	return err
}

// ExecX is like Exec, but panics if an error occurs.
func (iu *InstituteUpdate) ExecX(ctx context.Context) {
	if err := iu.Exec(ctx); err != nil {
		panic(err)
	}
}

func (iu *InstituteUpdate) sqlSave(ctx context.Context) (n int, err error) {
	_spec := &sqlgraph.UpdateSpec{
		Node: &sqlgraph.NodeSpec{
			Table:   institute.Table,
			Columns: institute.Columns,
			ID: &sqlgraph.FieldSpec{
				Type:   field.TypeInt,
				Column: institute.FieldID,
			},
		},
	}
	if ps := iu.mutation.predicates; len(ps) > 0 {
		_spec.Predicate = func(selector *sql.Selector) {
			for i := range ps {
				ps[i](selector)
			}
		}
	}
	if value, ok := iu.mutation.Name(); ok {
		_spec.Fields.Set = append(_spec.Fields.Set, &sqlgraph.FieldSpec{
			Type:   field.TypeString,
			Value:  value,
			Column: institute.FieldName,
		})
	}
	if iu.mutation.DirectionsCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   institute.DirectionsTable,
			Columns: []string{institute.DirectionsColumn},
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
				IDSpec: &sqlgraph.FieldSpec{
					Type:   field.TypeInt,
					Column: direction.FieldID,
				},
			},
		}
		_spec.Edges.Clear = append(_spec.Edges.Clear, edge)
	}
	if nodes := iu.mutation.RemovedDirectionsIDs(); len(nodes) > 0 && !iu.mutation.DirectionsCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   institute.DirectionsTable,
			Columns: []string{institute.DirectionsColumn},
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
				IDSpec: &sqlgraph.FieldSpec{
					Type:   field.TypeInt,
					Column: direction.FieldID,
				},
			},
		}
		for _, k := range nodes {
			edge.Target.Nodes = append(edge.Target.Nodes, k)
		}
		_spec.Edges.Clear = append(_spec.Edges.Clear, edge)
	}
	if nodes := iu.mutation.DirectionsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   institute.DirectionsTable,
			Columns: []string{institute.DirectionsColumn},
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
				IDSpec: &sqlgraph.FieldSpec{
					Type:   field.TypeInt,
					Column: direction.FieldID,
				},
			},
		}
		for _, k := range nodes {
			edge.Target.Nodes = append(edge.Target.Nodes, k)
		}
		_spec.Edges.Add = append(_spec.Edges.Add, edge)
	}
	if iu.mutation.AdminsCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   institute.AdminsTable,
			Columns: []string{institute.AdminsColumn},
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
				IDSpec: &sqlgraph.FieldSpec{
					Type:   field.TypeInt,
					Column: admin.FieldID,
				},
			},
		}
		_spec.Edges.Clear = append(_spec.Edges.Clear, edge)
	}
	if nodes := iu.mutation.RemovedAdminsIDs(); len(nodes) > 0 && !iu.mutation.AdminsCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   institute.AdminsTable,
			Columns: []string{institute.AdminsColumn},
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
				IDSpec: &sqlgraph.FieldSpec{
					Type:   field.TypeInt,
					Column: admin.FieldID,
				},
			},
		}
		for _, k := range nodes {
			edge.Target.Nodes = append(edge.Target.Nodes, k)
		}
		_spec.Edges.Clear = append(_spec.Edges.Clear, edge)
	}
	if nodes := iu.mutation.AdminsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   institute.AdminsTable,
			Columns: []string{institute.AdminsColumn},
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
				IDSpec: &sqlgraph.FieldSpec{
					Type:   field.TypeInt,
					Column: admin.FieldID,
				},
			},
		}
		for _, k := range nodes {
			edge.Target.Nodes = append(edge.Target.Nodes, k)
		}
		_spec.Edges.Add = append(_spec.Edges.Add, edge)
	}
	if n, err = sqlgraph.UpdateNodes(ctx, iu.driver, _spec); err != nil {
		if _, ok := err.(*sqlgraph.NotFoundError); ok {
			err = &NotFoundError{institute.Label}
		} else if sqlgraph.IsConstraintError(err) {
			err = &ConstraintError{err.Error(), err}
		}
		return 0, err
	}
	return n, nil
}

// InstituteUpdateOne is the builder for updating a single Institute entity.
type InstituteUpdateOne struct {
	config
	fields   []string
	hooks    []Hook
	mutation *InstituteMutation
}

// SetName sets the "name" field.
func (iuo *InstituteUpdateOne) SetName(s string) *InstituteUpdateOne {
	iuo.mutation.SetName(s)
	return iuo
}

// AddDirectionIDs adds the "Directions" edge to the Direction entity by IDs.
func (iuo *InstituteUpdateOne) AddDirectionIDs(ids ...int) *InstituteUpdateOne {
	iuo.mutation.AddDirectionIDs(ids...)
	return iuo
}

// AddDirections adds the "Directions" edges to the Direction entity.
func (iuo *InstituteUpdateOne) AddDirections(d ...*Direction) *InstituteUpdateOne {
	ids := make([]int, len(d))
	for i := range d {
		ids[i] = d[i].ID
	}
	return iuo.AddDirectionIDs(ids...)
}

// AddAdminIDs adds the "Admins" edge to the Admin entity by IDs.
func (iuo *InstituteUpdateOne) AddAdminIDs(ids ...int) *InstituteUpdateOne {
	iuo.mutation.AddAdminIDs(ids...)
	return iuo
}

// AddAdmins adds the "Admins" edges to the Admin entity.
func (iuo *InstituteUpdateOne) AddAdmins(a ...*Admin) *InstituteUpdateOne {
	ids := make([]int, len(a))
	for i := range a {
		ids[i] = a[i].ID
	}
	return iuo.AddAdminIDs(ids...)
}

// Mutation returns the InstituteMutation object of the builder.
func (iuo *InstituteUpdateOne) Mutation() *InstituteMutation {
	return iuo.mutation
}

// ClearDirections clears all "Directions" edges to the Direction entity.
func (iuo *InstituteUpdateOne) ClearDirections() *InstituteUpdateOne {
	iuo.mutation.ClearDirections()
	return iuo
}

// RemoveDirectionIDs removes the "Directions" edge to Direction entities by IDs.
func (iuo *InstituteUpdateOne) RemoveDirectionIDs(ids ...int) *InstituteUpdateOne {
	iuo.mutation.RemoveDirectionIDs(ids...)
	return iuo
}

// RemoveDirections removes "Directions" edges to Direction entities.
func (iuo *InstituteUpdateOne) RemoveDirections(d ...*Direction) *InstituteUpdateOne {
	ids := make([]int, len(d))
	for i := range d {
		ids[i] = d[i].ID
	}
	return iuo.RemoveDirectionIDs(ids...)
}

// ClearAdmins clears all "Admins" edges to the Admin entity.
func (iuo *InstituteUpdateOne) ClearAdmins() *InstituteUpdateOne {
	iuo.mutation.ClearAdmins()
	return iuo
}

// RemoveAdminIDs removes the "Admins" edge to Admin entities by IDs.
func (iuo *InstituteUpdateOne) RemoveAdminIDs(ids ...int) *InstituteUpdateOne {
	iuo.mutation.RemoveAdminIDs(ids...)
	return iuo
}

// RemoveAdmins removes "Admins" edges to Admin entities.
func (iuo *InstituteUpdateOne) RemoveAdmins(a ...*Admin) *InstituteUpdateOne {
	ids := make([]int, len(a))
	for i := range a {
		ids[i] = a[i].ID
	}
	return iuo.RemoveAdminIDs(ids...)
}

// Select allows selecting one or more fields (columns) of the returned entity.
// The default is selecting all fields defined in the entity schema.
func (iuo *InstituteUpdateOne) Select(field string, fields ...string) *InstituteUpdateOne {
	iuo.fields = append([]string{field}, fields...)
	return iuo
}

// Save executes the query and returns the updated Institute entity.
func (iuo *InstituteUpdateOne) Save(ctx context.Context) (*Institute, error) {
	var (
		err  error
		node *Institute
	)
	if len(iuo.hooks) == 0 {
		node, err = iuo.sqlSave(ctx)
	} else {
		var mut Mutator = MutateFunc(func(ctx context.Context, m Mutation) (Value, error) {
			mutation, ok := m.(*InstituteMutation)
			if !ok {
				return nil, fmt.Errorf("unexpected mutation type %T", m)
			}
			iuo.mutation = mutation
			node, err = iuo.sqlSave(ctx)
			mutation.done = true
			return node, err
		})
		for i := len(iuo.hooks) - 1; i >= 0; i-- {
			if iuo.hooks[i] == nil {
				return nil, fmt.Errorf("ent: uninitialized hook (forgotten import ent/runtime?)")
			}
			mut = iuo.hooks[i](mut)
		}
		if _, err := mut.Mutate(ctx, iuo.mutation); err != nil {
			return nil, err
		}
	}
	return node, err
}

// SaveX is like Save, but panics if an error occurs.
func (iuo *InstituteUpdateOne) SaveX(ctx context.Context) *Institute {
	node, err := iuo.Save(ctx)
	if err != nil {
		panic(err)
	}
	return node
}

// Exec executes the query on the entity.
func (iuo *InstituteUpdateOne) Exec(ctx context.Context) error {
	_, err := iuo.Save(ctx)
	return err
}

// ExecX is like Exec, but panics if an error occurs.
func (iuo *InstituteUpdateOne) ExecX(ctx context.Context) {
	if err := iuo.Exec(ctx); err != nil {
		panic(err)
	}
}

func (iuo *InstituteUpdateOne) sqlSave(ctx context.Context) (_node *Institute, err error) {
	_spec := &sqlgraph.UpdateSpec{
		Node: &sqlgraph.NodeSpec{
			Table:   institute.Table,
			Columns: institute.Columns,
			ID: &sqlgraph.FieldSpec{
				Type:   field.TypeInt,
				Column: institute.FieldID,
			},
		},
	}
	id, ok := iuo.mutation.ID()
	if !ok {
		return nil, &ValidationError{Name: "ID", err: fmt.Errorf("missing Institute.ID for update")}
	}
	_spec.Node.ID.Value = id
	if fields := iuo.fields; len(fields) > 0 {
		_spec.Node.Columns = make([]string, 0, len(fields))
		_spec.Node.Columns = append(_spec.Node.Columns, institute.FieldID)
		for _, f := range fields {
			if !institute.ValidColumn(f) {
				return nil, &ValidationError{Name: f, err: fmt.Errorf("ent: invalid field %q for query", f)}
			}
			if f != institute.FieldID {
				_spec.Node.Columns = append(_spec.Node.Columns, f)
			}
		}
	}
	if ps := iuo.mutation.predicates; len(ps) > 0 {
		_spec.Predicate = func(selector *sql.Selector) {
			for i := range ps {
				ps[i](selector)
			}
		}
	}
	if value, ok := iuo.mutation.Name(); ok {
		_spec.Fields.Set = append(_spec.Fields.Set, &sqlgraph.FieldSpec{
			Type:   field.TypeString,
			Value:  value,
			Column: institute.FieldName,
		})
	}
	if iuo.mutation.DirectionsCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   institute.DirectionsTable,
			Columns: []string{institute.DirectionsColumn},
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
				IDSpec: &sqlgraph.FieldSpec{
					Type:   field.TypeInt,
					Column: direction.FieldID,
				},
			},
		}
		_spec.Edges.Clear = append(_spec.Edges.Clear, edge)
	}
	if nodes := iuo.mutation.RemovedDirectionsIDs(); len(nodes) > 0 && !iuo.mutation.DirectionsCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   institute.DirectionsTable,
			Columns: []string{institute.DirectionsColumn},
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
				IDSpec: &sqlgraph.FieldSpec{
					Type:   field.TypeInt,
					Column: direction.FieldID,
				},
			},
		}
		for _, k := range nodes {
			edge.Target.Nodes = append(edge.Target.Nodes, k)
		}
		_spec.Edges.Clear = append(_spec.Edges.Clear, edge)
	}
	if nodes := iuo.mutation.DirectionsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   institute.DirectionsTable,
			Columns: []string{institute.DirectionsColumn},
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
				IDSpec: &sqlgraph.FieldSpec{
					Type:   field.TypeInt,
					Column: direction.FieldID,
				},
			},
		}
		for _, k := range nodes {
			edge.Target.Nodes = append(edge.Target.Nodes, k)
		}
		_spec.Edges.Add = append(_spec.Edges.Add, edge)
	}
	if iuo.mutation.AdminsCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   institute.AdminsTable,
			Columns: []string{institute.AdminsColumn},
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
				IDSpec: &sqlgraph.FieldSpec{
					Type:   field.TypeInt,
					Column: admin.FieldID,
				},
			},
		}
		_spec.Edges.Clear = append(_spec.Edges.Clear, edge)
	}
	if nodes := iuo.mutation.RemovedAdminsIDs(); len(nodes) > 0 && !iuo.mutation.AdminsCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   institute.AdminsTable,
			Columns: []string{institute.AdminsColumn},
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
				IDSpec: &sqlgraph.FieldSpec{
					Type:   field.TypeInt,
					Column: admin.FieldID,
				},
			},
		}
		for _, k := range nodes {
			edge.Target.Nodes = append(edge.Target.Nodes, k)
		}
		_spec.Edges.Clear = append(_spec.Edges.Clear, edge)
	}
	if nodes := iuo.mutation.AdminsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   institute.AdminsTable,
			Columns: []string{institute.AdminsColumn},
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
				IDSpec: &sqlgraph.FieldSpec{
					Type:   field.TypeInt,
					Column: admin.FieldID,
				},
			},
		}
		for _, k := range nodes {
			edge.Target.Nodes = append(edge.Target.Nodes, k)
		}
		_spec.Edges.Add = append(_spec.Edges.Add, edge)
	}
	_node = &Institute{config: iuo.config}
	_spec.Assign = _node.assignValues
	_spec.ScanValues = _node.scanValues
	if err = sqlgraph.UpdateNode(ctx, iuo.driver, _spec); err != nil {
		if _, ok := err.(*sqlgraph.NotFoundError); ok {
			err = &NotFoundError{institute.Label}
		} else if sqlgraph.IsConstraintError(err) {
			err = &ConstraintError{err.Error(), err}
		}
		return nil, err
	}
	return _node, nil
}
